...
export const handlers = [
  ...
  http.post(`${API_BASE_URL}/stores`, async ({ request }) => {
    ...
    if (!payload.name?.trim() || !payload.address?.trim()) {
      return HttpResponse.text('Nome e endereço são obrigatórios', { status: 400 });
    }
  ...
  http.post(`${API_BASE_URL}/products`, async ({ request }) => {
    const payload = (await request.json()) as Pick<Product, 'storeId' | 'name' | 'category' | 'price'>;

    if (!payload.storeId || !payload.name?.trim() || !payload.category?.trim() || payload.price <= 0) {
  ...
  http.put(`${API_BASE_URL}/products/:productId`, async ({ request, params }) => {
    const payload = (await request.json()) as Pick<Product, 'name' | 'category' | 'price'>;
    ...
    productsDb[index] = {
      ...
      price: payload.price && payload.price > 0 ? payload.price : productsDb[index].price,
    };
